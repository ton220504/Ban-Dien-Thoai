...
const SPTheoLoai = () => {
    ...
    useEffect(() => {
        (async () => {
            const result = await ProductService.getListProductCategory(id, currentPage, limit);
            const result_menus = await menuService.getshow(id);

            setProducts(result.products);
            setMenus(result_menus.menus);
    ...
    return (
        <div className="container">
            {menus && menus.length > 0 && menus.map((men, index) => {
                return (
                    <h2 key={index} className="text-center my-4"><strong className="text-danger">{men.name}</strong></h2>
                );
            })}
            <div className="row" >
